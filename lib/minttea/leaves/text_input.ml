...
let placeholder_view t =
  ...
  for i = 0 to String.length t.placeholder - 1 do
    ...
    let txt =
      if i = 0
      then Cursor.view t.cursor ~text_style:t.placeholder_style s
...
let view t =
  if String.length t.value = 0
  then placeholder_view t
  else (
    let text_style = Spices.(t.text_style |> build) in
    let result = ref "" in
    for i = 0 to String.length t.value - 1 do
      ...
      let txt =
        if i = t.pos
        then Cursor.view t.cursor ~text_style:t.text_style s
        else text_style "%s" s
      in
      result := !result ^ txt
    done;
...
let update t (e : Minttea.Event.t) =
  ...
  | _ ->
    ...
    let updated_cursor =
      if time_since_last_action <= resume_blink_after
      then Cursor.enable_blink t.cursor
      else t.cursor
    in
