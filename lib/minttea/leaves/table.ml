...
let update t (e : action) =
  ...
  let go_down t steps =
    ...
    let start_of_frame =
      if cursor > t.start_of_frame + t.height_of_frame - 1
      then cursor - t.height_of_frame + 1
...
let truncate_or_pad_unicode ~width str =
  (* Truncates a Unicode string to a target width with an ellipsis or pads it
     with spaces on the right. *)
  let uuseg_string_length s =
    (* TODO(@sabine): we probably don't want the uuseg dependency or move
       this somewhere where it actually belongs. Also for uuseg_string_sub,
    ...
    let inner s start n acc =
      Uuseg_string.fold_utf_8
        ...
        (fun (pos, collected, s) c ->
          if pos >= start && collected < n
          then pos + 1, collected + 1, s ^ c
  ...
  if len > width
  then uuseg_string_sub str 0 (width - 1) ^ "…"
...
let view t =
  ...
  let rows =
    let get_visible_rows t =
      t.rows
      |> List.to_seq
      |> Seq.drop t.start_of_frame
      |> Seq.take t.height_of_frame
      |> List.of_seq
    in
    let render_row i row =
      ...
      if i = t.cursor - t.start_of_frame
      then (t.styles.cursor |> Spices.build) "%s" row
      else row
    in
