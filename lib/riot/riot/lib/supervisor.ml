...
type sup_request =
  | List_children_req of
      { reply : Pid.t
      ; ref : unit Ref.t
      }

type sup_response =
  | List_children_res of
      { children : Pid.t list
      ; ref : unit Ref.t
      }
...
let rec loop state =
  ...
  let selector : _ Message.selector =
    fun msg ->
    match msg with
    | Process.Messages.Exit (pid, reason)
      when List.mem_assoc pid state.children ->
...
and handle_child_exit pid _reason state =
  ...
  | `terminate ->
    trace (fun f ->
      f
        "Supervisor %a reached max restarts of %d"
        Pid.pp
        (self ())
        state.restart_limit)
;;
...
let start_supervisor state =
  trace (fun f ->
    f
      "Initializing supervisor %a with %d child specs"
      Pid.pp
      (self ())
