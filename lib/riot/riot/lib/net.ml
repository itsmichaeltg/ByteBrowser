...
module Tcp_stream = struct
  ...
  let rec receive ?timeout ~bufs t =
    trace (fun f ->
      f
        "receiving up to %d octets from %a"
        (Rio.Iovec.length bufs)
        Socket.pp
        t);
  ...
  let rec send ?timeout ~bufs t =
    ...
    | Error `Would_block ->
      ...
      syscall ?timeout "send" Interest.writable (to_source t)
      @@ fun () -> send ?timeout ~bufs t
    | Error err -> Error err
  ;;
