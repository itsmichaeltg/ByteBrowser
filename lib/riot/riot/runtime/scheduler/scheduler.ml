...
let shutdown pool status =
  let rec wake_up_scheduler (sch : t) =
    if Mutex.try_lock sch.idle_mutex
    then (
...
module Scheduler = struct
  ...
  let add_to_run_queue (sch : t) (proc : Process.t) =
    Mutex.protect sch.idle_mutex
    @@ fun () ->
    Proc_queue.queue sch.run_queue proc;
    Condition.signal sch.idle_condition;
    Log.trace (fun f ->
      f
        "Adding process to run_queue queue[%d]: %a"
  ...
  let awake_process pool (proc : Process.t) =
    List.iter
      (fun (sch : t) ->
        if Scheduler_uid.equal sch.uid (Atomic.get proc.sid)
        then add_to_run_queue sch proc)
      pool.schedulers
  ;;
  ...
  let handle_receive
    ...
    ~selector
    =
    Trace.handle_receive_span
    @@ fun () ->
    let open Proc_state in
    (* When a timeout is specified, we want to create it in the timer wheel,
       and save the timer reference in the process.

       That way, whenever we gets re-scheduled to run this specific `Receive`
       effect we can check on the Process to see if it has timed out. *)
    let should_timeout, _next_step =
      ...
      | Some timeout, _ ->
        ...
        Log.debug (fun f ->
          f
            "Process %a: process receive timeout? %b"
            Pid.pp
            (Process.pid proc)
            finished);
        finished, `receiving
    ...
    Log.trace (fun f ->
      f
        "Process %a: receiving messages (timeout? %b | empty_mailbox? %b)"
        Pid.pp
        (Process.pid proc)
        should_timeout
    ...
    then (
      Log.trace (fun f ->
        f "Process %a: timed out" Pid.pp (Process.pid proc));
      Process.clear_receive_timeout proc;
      k (Discontinue Process.Exn.Receive_timeout))
    else if Process.has_empty_mailbox proc
    then (
      Log.trace (fun f ->
      ...
      (* NOTE(@leostera): we will use the current message count as fuel to
         stop the recursion. This count may increase as we are iterating, but
         that's okay. If we consume it entirely, the receive will just be
         delayed until the next scheduled run, and at that point we'll pick
         up any new messages. *)
      ...
      let rec go fuel =
        ...
        then k Delay
          ...
          | _, None ->
            ...
            k Delay
          (* when we have an explicit ref, we will skip any message that was
             created BEFORE this ref was created, which enables the selective
             receive.

             Any skipped messages will go in the same order as received into
             the save queue, which will be read after the mailbox is
             depleted. *)
          | Some ref, Some msg when Ref.is_newer ref msg.uid ->
            ...
            go (fuel - 1)
          (* we are special casing the process monitors here. if we receive a
             process down but we already have removed the monitor, then we
             will simply ignore this message. *)
          | ( _
            , Some
                (Message.
                   { msg =
            ...
            Process.clear_receive_timeout proc;
            if Process.is_monitored_by_pid proc mon_pid
            then (
          ...
          (* lastly, if we have a ref and the mesasge is newer than the ref,
             and when we don't have a ref, we just pop the message and
             continue with it *)
          | _, Some msg ->
  ...
  let handle_syscall
    ...
    Log.debug (fun f ->
      f
        "handle_syscall %s with %a (timeout? %b)"
        name
        Process.pp
        proc
        should_timeout);
    if Process.is_exited proc
    then k (Discontinue Process.Exn.Terminated)
      ...
      | None ->
        ...
        (match timeout with
         ...
         | `after s ->
           Log.debug (fun f ->
             f
               "Process %a: creating syscall timeout"
               Pid.pp
               (Process.pid proc));
           let timeout =
             Timer_wheel.make_timer sch.timers s `one_off (fun () ->
  ...
  let handle_exit_proc pool (sch : t) proc (reason : Process.exit_reason) =
    ...
    List.iter
      (fun mon_pid ->
        ...
        | Some mon_proc when Process.is_exited mon_proc ->
          Log.debug (fun f ->
            f
              "monitoring process %a is dead, nothing to do"
              Pid.pp
              mon_proc.pid)
        | Some mon_proc ->
          Log.debug (fun f ->
    ...
    Log.debug (fun f ->
      f
        "terminating %d processes linked to %a"
        (List.length linked_pids)
        Pid.pp
        proc.pid);
    List.iter
      (fun link_pid ->
        ...
        | Some linked_proc when Process.is_exited linked_proc ->
          Log.debug (fun f ->
            f
              "linked process %a is already dead, nothing to do"
              Pid.pp
              linked_proc.pid)
        | Some linked_proc ->
  ...
  let handle_run_proc pool (sch : t) proc =
    ...
    try
      ...
      | _ when Process.is_waiting_io proc ->
        Log.trace (fun f ->
          f
            "Process %a hibernated (will resume): %a"
            Pid.pp
            proc.pid
            Process.pp
            proc)
      | Proc_state.Suspended _ | Proc_state.Unhandled _ ->
        Log.trace (fun f ->
          f
            "Process %a suspended (will resume): %a"
            Pid.pp
            proc.pid
            Process.pp
            proc);
  ...
  let steal_processes pool (us : t) =
    List.iter
      (fun (them : t) ->
        ...
        if Scheduler_uid.equal us.uid them.uid
        then ()
        else (
...
module Pool = struct
  ...
  let setup () =
    (* NOTE(@leostera): we want the Net subsystem to be able to write to
       closed sockets and handle that as a regular value rather than as a
       signal. *)
  ...
  let make ?(rnd = Random.State.make_self_init ()) ~domains ~main () =
    ...
    let spawn (scheduler : t) =
      Stdlib.Domain.spawn (fun () ->
        ...
        | exn ->
          Log.error (fun f ->
            f
              "Scheduler.run exception: %s due to: %s%!"
    ...
    let io_thread =
      Stdlib.Domain.spawn (fun () ->
        try Io_scheduler.run pool io_scheduler () with
        | exn ->
          Log.error (fun f ->
            f
              "Io_scheduler.run exception: %s due to: %s%!"
              (Printexc.to_string exn)
              (Printexc.raw_backtrace_to_string
                 (Printexc.get_raw_backtrace ())));
          shutdown pool 2)
