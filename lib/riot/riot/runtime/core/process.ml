...
let make sid fn =
  ...
  Log.debug (fun f ->
    f
      "Making process with pid=%a and size=%d"
      Pid.pp
      pid
...
let rec pp ppf t =
  Format.fprintf
    ppf
    "Process %a { sid = %a; state = %a; messages = %d; flags = %a }"
    Pid.pp
    t.pid
    Scheduler_uid.pp
    (Atomic.get t.sid)
...
and pp_reason ppf (t : exit_reason) =
  ...
  | Exception exn ->
    Format.fprintf ppf "Exception: %s" (Printexc.to_string exn)

and pp_flags ppf (t : process_flags) =
  Format.fprintf
    ppf
    "{ trap_exits=%b; priority=%s }"
    (Atomic.get t.trap_exits)
...
let is_alive t =
  ...
  | Uninitialized | Runnable | Waiting_message | Waiting_io _ | Running ->
    true
;;
...
let rec set_syscall_timeout t timeout =
  ...
  if Atomic.compare_and_set t.syscall_timeout last_timeout (Some timeout)
  then ()
...
let rec clear_syscall_timeout t =
  ...
  if Atomic.compare_and_set t.syscall_timeout last_timeout None
  then ()
...
let rec set_receive_timeout t timeout =
  ...
  if Atomic.compare_and_set t.recv_timeout last_timeout (Some timeout)
  then ()
...
let rec clear_receive_timeout t =
  let last_timeout = Atomic.get t.recv_timeout in
  if Atomic.compare_and_set t.recv_timeout last_timeout None
  then ()
...
let rec mark_as_awaiting_io t syscall token source =
  ...
  then
    Log.trace (fun f ->
      f "Process %a: marked as waiting for io" Pid.pp t.pid)
...
let rec mark_as_awaiting_message t =
  if is_exited t then raise (Process_reviving_is_forbidden t);
  let old_state = Atomic.get t.state in
  if Atomic.compare_and_set t.state old_state Waiting_message
  then
    Log.trace (fun f ->
...
let rec mark_as_running t =
  if is_exited t then raise (Process_reviving_is_forbidden t);
  let old_state = Atomic.get t.state in
  if Atomic.compare_and_set t.state old_state Running
  then Log.trace (fun f -> f "Process %a: marked as running" Pid.pp t.pid)
...
let rec mark_as_runnable t =
  if is_exited t then raise (Process_reviving_is_forbidden t);
  let old_state = Atomic.get t.state in
  if Atomic.compare_and_set t.state old_state Runnable
  then Log.trace (fun f -> f "Process %a: marked as runnable" Pid.pp t.pid)
...
let rec mark_as_exited t reason =
  ...
  then ()
    ...
    let old_state = Atomic.get t.state in
    if Atomic.compare_and_set t.state old_state (Exited reason)
    then
      Log.trace (fun f ->
        f
          "Process %a: marked as exited with reason %a"
          Pid.pp
          t.pid
          pp_reason
          reason)
...
let rec mark_as_finalized t =
  let old_state = Atomic.get t.state in
  if Atomic.compare_and_set t.state old_state Finalized
  then Log.trace (fun f -> f "Process %a: marked as finalized" Pid.pp t.pid)
...
let rec set_flag t flag =
  ...
  | Trap_exit v ->
    ...
    if Atomic.compare_and_set t.flags.trap_exits old_flag v
    then ()
    else set_flag t flag
  | Priority p ->
    let old_flag = Atomic.get t.flags.priority in
    if Atomic.compare_and_set t.flags.priority old_flag p
    then ()
    else set_flag t flag
;;
...
let rec add_link t link =
  ...
  if Atomic.compare_and_set t.links old_links new_links
  then (
    Log.trace (fun f ->
