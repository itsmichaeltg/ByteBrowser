...
(* NOTE(leostera): to send a message, we will find the receiver process in
   the process table and queue at the back of their mailbox *)
let send pid msg =
  let pool = _get_pool () in
  match Proc_table.get pool.processes pid with
  | Some proc ->
  ...
  | None ->
    Log.debug (fun f ->
      f "COULD NOT DELIVER message from %a to %a" Pid.pp (self ()) Pid.pp pid)
;;
...
let _spawn
  ...
  ?(do_link = false)
  ?(pool = _get_pool ())
  ?(scheduler = Scheduler.get_random_scheduler pool)
  fn
  =
  let proc =
    Process.make scheduler.uid (fun () ->
      ...
      | exn ->
        Log.error (fun f ->
          f
            "Process %a died with unhandled exception %s:\n%s"
            Pid.pp
            (self ())
...
let wait_pids pids =
  ...
  (* Immediately after monitoring, we want to make sure we remove from the
     list all the pids that are already terminated, since we won't receive
     monitoring messages for those. *)
  let pool = _get_pool () in
  let pids =
    List.filter
      (fun pid ->
