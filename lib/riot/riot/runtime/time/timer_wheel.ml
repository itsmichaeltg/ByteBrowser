...
module Timer = struct
  ...
  let pp fmt t =
    ...
    Format.fprintf
      fmt
      "Timer { id=%a; started_at=%a; ends_at=%a; mode=%s }"
      Ref.pp
      t.id
      Mtime.pp
      t.started_at
...
let create () =
  ...
  ; timer_count = Atomic.make 0
  }
;;

let can_tick t = Atomic.get t.timer_count > 0
let size t = Atomic.get t.timer_count
...
let run_timer now timer =
  let open Timer in
  if Timer.is_finished timer
  then (
    Log.debug (fun f ->
      f "Removing already finished timer: %a" Timer.pp timer);
    None)
  else (
    let ends_at =
    ...
    in
    let timeout =
      Mtime.is_later now ~than:ends_at || Mtime.equal now ends_at
    in
    Log.debug (fun f ->
      f "now(%a) > ends_at(%a) = %b" Mtime.pp now Mtime.pp ends_at timeout);
    Log.debug (fun f ->
      f
        "Running timer %a with ends_at=%a -> timeout? %b"
        Timer.pp
        timer
        Mtime.pp
        ends_at
...
let tick t =
  ...
  Log.trace (fun f ->
    f
      "Done Ticking (%d timers left) %a"
      (Atomic.get t.timer_count)
      Mtime.pp
      (Mtime_clock.now ()))
;;
