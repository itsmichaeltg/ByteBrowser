...
let server port socket =
  ...
  let rec echo () =
    Logger.debug (fun f ->
      f
        "Reading from client client %a (%a)"
        Net.Addr.pp
        addr
        Net.Socket.pp
        conn);
...
let client server_port main =
  ...
  let rec send_loop n =
    if n = 0
    then Logger.error (fun f -> f "client retried too many times")
    else (
      match IO.write_owned_vectored ~bufs writer with
  ...
  if len = 0
  then send main (Received "empty paylaod")
...
let () =
  ...
  let server =
    spawn (fun () ->
      try server port socket with
      | SSL.Tls_failure failure ->
        Logger.error (fun f ->
          f "server error: %a" Tls.Engine.pp_failure failure))
  in
  let client =
    spawn (fun () ->
      try client port main with
      | SSL.Tls_failure failure ->
        Logger.error (fun f ->
