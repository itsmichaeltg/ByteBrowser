...
let main () =
  ...
  let rec check_loop iters =
    ...
    then (
      Logger.error (fun f ->
        f
          "process_stealing_test: process was not stolen by another \
           scheduler");
      raise Fail);
    let current_sid = get_sid pid in
    if not (Core.Scheduler_uid.equal last_sid current_sid)
    then Logger.info (fun f -> f "process_stealing_test: OK")
