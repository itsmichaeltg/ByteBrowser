...
let test_with_buffer capacity =
  ...
  if String.length final_str = 57946
  then Logger.info (fun f -> f "io_reader_large_test(%d): OK" capacity)
  else (
    Logger.error (fun f ->
      f
        "io_reader_large_test(%d): %d unexpected input  %S %S %S"
...
let () =
  ...
  test_with_buffer 7;
  (* larger than the target but smaller than the and smaller than the
     sourcesrc *)
  test_with_buffer 10;
