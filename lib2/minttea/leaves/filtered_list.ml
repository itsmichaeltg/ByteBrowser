...
let update event (model : t) =
  ...
  | Event.KeyDown ((Key "s" | Space), _) ->
    ...
    { model with
      elements =
        List.mapi
          (fun idx (s, e) -> if idx = model.cursor then not s, e else s, e)
          model.elements
    }
...
let view (model : t) =
  ...
  let format_row (idx, selected, element) =
    let cursor =
      if model.cursor = idx
      then model.cursor_string
      else String.make (String.length model.cursor_string) ' '
    in
    let bullet =
      if selected
      then Format.sprintf "[%2d]" (idx + 1)
  ...
  let lst =
    if List.length rows < model.max_height
    then
      String.concat
        "\n"
...
let filter model predicate =
  let model =
    { model with
      elements =
        List.filteri (fun idx (_, e) -> predicate idx e) model.elements
    }
  in
...
let get_selection model =
  model.elements
  |> List.filter (fun (selected, _) -> selected)
  |> List.map snd
;;
