...
let%expect_test "navigate-right" =
  ...
  print_endline (Visualize_helper.visualize newer_model.choices.matrix ~current_directory:"/home" ~path_to_be_underlined:newer_model.current_path);
  let newest_model = Navigate.State.get_updated_model_for_right new_model in
  print_endline (Visualize_helper.visualize newest_model.choices.matrix ~current_directory:"/home" ~path_to_be_underlined:newest_model.current_path);
  [%expect
    {|
    .
    [0m[0m|__ 📁[;0;36mhome
