...
let%expect_test "files_in_dir" =
    print_s
      [%sexp
        (Visualize.Adjacency_matrix.get_files_in_dir "/home/ubuntu/jsip-final-project/test_dir"
         : string list)];
  ...
  [%expect
    ...
    Visualize.visualize ~max_depth:10 ~origin:"/home/ubuntu/jsip-final-project/test_dir";
    [%expect
      " \n\
      \ .\n\
  ...
  let%expect_test "get limited children to tree" =
    ...
    let origin, num_to_show, max_depth = "/home/ubuntu/jsip-final-project", 3, 100 in
    let result = Visualize.Adjacency_matrix.get_limited_adjacency_matrix initial_tree ~origin ~num_to_show ~max_depth in
    print_endline (Visualize_helper.visualize result.matrix ~current_directory:origin ~path_to_be_underlined:origin);
    [%expect " 
 .
